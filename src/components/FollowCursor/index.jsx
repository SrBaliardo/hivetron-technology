  ...
  useEffect(() => {
    if (!isMobile() && enableTilt) {
      const handleMouseMove = (event) => {
        ...
        const containerCenterX = rect.left + rect.width / 1.66;
        const containerCenterY = rect.top + rect.height / 100;

        let { clientX: px, clientY: py } = event;

        const cardWidthNum = parseFloat(cardWidth);
        const maxRight = window.innerWidth - cardWidthNum;
        if (px > maxRight) px = maxRight;
        if (px < 0) px = 0;

        const xPos = px - containerCenterX;
        const yPos = py - containerCenterY;

        const calculatedWidth = container.offsetWidth * (cardWidthNum / 180);
